#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Genera notas/c4-parque-nieve-2026(.html|-en|-pt|-zh) usando las traducciones
ya escritas en historial.json (sin API, sin truncado). Mismo patrón que
_gen_variantes_datacenter.py."""
import os, sys, json
...
BID = "c4-parque-nieve-2026"
LANGS = ["en", "pt", "zh"]

historial = json.load(open(os.path.join(BASE, "historial.json"), encoding="utf-8"))
entry = next(n for n in historial if n["id"] == BID)

pool = []
for src in ("historial.json", "propios.json", "propios_historial.json", "historias.json",
            "noticias.json", "turismo.json", "deportes_feed.json", "negocios.json",
            "cultura.json", "guias.json"):
...
for lang in LANGS:
    synth.append({
        ...
        "url_original": entry.get("url_original", ""),
        "pais": entry.get("pais", ""),
...
print("Sitemap regenerado. LISTO")
